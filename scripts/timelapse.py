...
def BuildImgList(dir, ext):
  ...
  with open(dir + "/" + DEF_FLIST, 'w') as flist:
    for fname in img_list:
      flist.write(dir + "/" + fname + "\n")

def ExecVideoBuild(dir, t_date):
  cmd = ["ffmpeg", "-f concat -i ${} -r 10 -an -crf 28 -c:v libx265 -preset veryfast -pix_fmt yuv420p ${}.mp4".format(
    dir + "/" + DEF_IMG_DNAME + "/" + t_date + "/" + DEF_FLIST, dir + "/" + DEF_MOV_DNAME + "/" + t_date)]
  subprocess.call(cmd)
